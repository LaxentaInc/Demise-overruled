...
public class JumpResetRecorder extends Module {
    ...
    public void onDisable() {
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(saveFile))) {
            writer.println("Tick | VelRecv | HurtTime | OnGround | MotionXZ | MotionY | IsJumping | ResetCD | MoveFwd");
            writer.println("---------------------------------------------------------------------------------------------");
    ...
    public void onUpdate(UpdateEvent e) {
        ...
        String entry = String.format(
            "%-4d | %-7b | %-8d | %-8b | %-8.3f | %-7.3f | %-9b | %-7d | %-7.1f",
            tickCounter, velRecv, hurtTime, onGround, motionXZ, motionY, isJumping, resetCD, moveFwd
        );
